...
| **ItemSound**         | string | "generic" | 物品交互音效类型      | [itemSound 可以赋值的值](../常用常量/字符串常量/itemSound 可以赋值的值.md) | 可选   |
...
| **EffectsHealth**    | Dictionary<HealthFactor, EffectsHealthProperties>     | 生命值恢复效果配置                                        | [HealthFactor](https://github.com/sp-tarkov/server-csharp/blob/main/Libraries/SPTarkov.Server.Core/Models/Enums/HealthFactor.cs), 一般设置为\[\]\(你没看错, 在药品中就是空的列表\) | 可选   |
| **EffectsDamage**    | Dictionary<DamageEffectType, EffectsDamageProperties> | 伤害治疗效果配置                                          | [DamageEffectType](https://github.com/sp-tarkov/server-csharp/blob/main/Libraries/SPTarkov.Server.Core/Models/Enums/DamageEffectType.cs)是键 | 可选   |
