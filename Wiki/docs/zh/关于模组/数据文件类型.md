...
`propertyOverride`的数据类型为[TemplateItemProperties](https://github.com/sp-tarkov/server-csharp/blob/895d53262e32e2c6a7116048c1c85bfa7770a858/Libraries/SPTarkov.Server.Core/Models/Eft/Common/Tables/TemplateItem.cs#L71), 且优先级会低于其他封装过的字段(例如如果同时在`attributeInfo`和`propertyOverride`中赋值了`Weight`属性，模组会用`attributeInfo`的`Weight`属性覆盖`propertyOverride`的`Weight`属性)
