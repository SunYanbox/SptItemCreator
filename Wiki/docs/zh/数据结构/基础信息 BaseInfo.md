...
| Type             | "type"             | string                                                       | 可选     | "common"                                                | 物品类型标识（仅用于标识，不影响属性应用）             |
...
"baseInfo": {
    ...
    // 物品类型标识（可选），仅用于标识目的
    // 实际属性由赋值的 Info 字段（medicalInfo、drinkFoodInfo 等）决定
    "type": "drinkOrDrugs",
    // 物品在游戏中显示的名称
    "name": "食物模板",
...
> **向后兼容**：旧版本数据文件中的 `$type` 字段仍可被正确读取，但不会输出到新文件中。
