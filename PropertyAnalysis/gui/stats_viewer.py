...
class StatsViewerGUI:
    ...
    def create_widgets(self):
        """创建界面组件"""
        # 顶部标签
        title_label = tk.Label(self.parent, text=gettext.gettext("数据查看"), font=("Arial", 14, "bold"))
        ...
        self.status_label = tk.Label(status_frame, text=gettext.gettext("已加载数据条数: {}").format(data_count))
        ...
        # 左列: BaseClasses 区域
        bc_frame = tk.LabelFrame(main_container, text=gettext.gettext("BaseClasses 列表"), padx=10, pady=10)
        ...
        tk.Label(bc_list_frame, text=gettext.gettext("所有BaseClasses:")).pack(anchor='w')
        ...
        tk.Button(bc_button_frame, text=gettext.gettext("刷新列表"), command=self.refresh_base_classes).pack(side='left', padx=5)
        tk.Button(bc_button_frame, text=gettext.gettext("查看详情"), command=self.show_props_for_selected).pack(side='left', padx=5)
        
        # 右列: PropKeys 区域
        pk_frame = tk.LabelFrame(main_container, text=gettext.gettext("PropKeys 列表"), padx=10, pady=10)
        ...
        tk.Label(pk_list_frame, text=gettext.gettext("所有PropKeys:")).pack(anchor='w')
        ...
        tk.Button(pk_button_frame, text=gettext.gettext("刷新列表"), command=self.refresh_prop_keys).pack(side='left', padx=5)
    ...
    def refresh_base_classes(self):
        ...
        if self.stats_mgr:
            ...
            self.bc_listbox.insert(tk.END, gettext.gettext("未加载数据"))
    ...
    def refresh_prop_keys(self):
        ...
        if self.stats_mgr:
            ...
            self.pk_listbox.insert(tk.END, gettext.gettext("未加载数据"))
    ...
    def update_status_for_selected(self, bc_name: str) -> None:
        ...
        self.status_label.config(
            text=gettext.gettext("已加载数据条数: {} | 选中: {} | 属性: {} | 唯一属性: {}").format(
    ...
    def show_props_for_selected(self) -> Optional[tk.Toplevel]:
        ...
        selection = self.bc_listbox.curselection()
        if not selection:
            messagebox.showwarning(gettext.gettext("警告"), gettext.gettext("请先选择一个BaseClass"))
            return None
        
        bc_name = self.bc_listbox.get(selection[0])
        if not self.stats_mgr:
            messagebox.showerror(gettext.gettext("错误"), gettext.gettext("StatsManager未加载"))
            return None

        title = gettext.gettext("BaseClass 详情: {}").format(bc_name)
        ...
        tk.Button(button_frame, text=gettext.gettext("添加到标签页"),
                  command=lambda: add_to_tab(bc_name, self.stats_mgr)).pack(side='left', padx=5)

        tk.Button(button_frame, text=gettext.gettext("关闭窗口"),
